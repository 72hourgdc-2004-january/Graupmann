...
UINT CreateOpenGLFont(LPSTR strFontName, int height, UINT ID)	// Build Our Bitmap Font
{
	UINT	fontListID = ID;								// This will hold the base ID for our display list
	...
	hFont = CreateFont(	height,					// Our desired HEIGHT of the font
						...
						FF_DECORATIVE|FIXED_PITCH,	// The family and pitch of the font.  We don't care.
...
void PositionText(int x, int y, UINT Size)
{
	...
	y = g_rRect.bottom - FONT_HEIGHT - y;				// Calculate the weird screen position
...
void glDrawText(int x, int y, UINT ID, UINT Size, const char *strString, ...)
{
	...
	PositionText(x, y, Size);									// Call our own function to position the text on screen
	...
	glListBase(ID);							// This sets the lists base
