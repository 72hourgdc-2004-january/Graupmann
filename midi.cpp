...
bool MIDI::OpenMidi(char file_name[])
{
	...
	if(mciSendCommand(0, MCI_OPEN, MCI_OPEN_TYPE | MCI_OPEN_ELEMENT, (DWORD_PTR)&mci_open_params))
		return false;
...
bool MIDI::PlayMidi(HWND hwnd, bool repeat)
{
	...
	mciSendCommand(device_handle,MCI_SEEK,MCI_SEEK_TO_START,(DWORD_PTR)&seek_params);

	play_params.dwCallback = (DWORD_PTR)hwnd; // We know where the "MCI_NOTIFY message" is taken care of so we'll fill that in
	...
	if(mciSendCommand(device_handle,MCI_PLAY,MCI_NOTIFY,(DWORD_PTR)&play_params))
		return false;
...
bool MIDI::StopMidi()
{
	...
	if(mciSendCommand(device_handle,MCI_STOP,MCI_WAIT,(DWORD_PTR)&generic_params))
		return false;
...
bool MIDI::CloseMidi()
{
	...
	if(mciSendCommand(device_handle,MCI_CLOSE,MCI_WAIT,(DWORD_PTR)&close_params))
